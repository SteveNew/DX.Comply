...
procedure THtmlReportWriter.AddBuildEvidence(Lines: TStrings; const AData: TComplianceReportData);
var
  LEvidenceItem: TBuildEvidenceItem;
begin
  Lines.Add('<section><h2>Build Evidence</h2><table><thead><tr><th>Source</th><th>Display Name</th><th>Unit / Package</th><th>Detail</th></tr></thead><tbody>');
...
procedure THtmlReportWriter.AddCompositionEvidence(Lines: TStrings; const AData: TComplianceReportData);
var
  LUnit: TResolvedUnitInfo;
begin
  Lines.Add('<section><h2>Composition Evidence</h2><table><thead><tr><th>Unit</th><th>Origin</th><th>Evidence</th><th>Confidence</th><th>Location</th></tr></thead><tbody>');
  for LUnit in AData.CompositionEvidence.Units do
    Lines.Add(Format('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>', [
      EscapeHtml(SafeText(LUnit.UnitName)),
      EscapeHtml(UnitOriginKindToString(LUnit.OriginKind)),
      EscapeHtml(UnitEvidenceKindToString(LUnit.EvidenceKind)),
      EscapeHtml(ResolutionConfidenceToString(LUnit.Confidence)),
      EscapeHtml(SafeText(LUnit.ResolvedPath, SafeText(LUnit.ContainerPath)))]));
  Lines.Add('</tbody></table></section>');
end;
...
begin
  ...
  try
    ...
    Lines.Add('<title>DX.Comply Human-Readable Compliance Report</title>');
    Lines.Add('<style>body{font-family:Segoe UI,Arial,sans-serif;margin:0;background:#0f172a;color:#e2e8f0}main{max-width:1200px;margin:0 auto;padding:32px}h1,h2,h3{color:#f8fafc}a{color:#38bdf8}section{background:#111827;border:1px solid #334155;border-radius:12px;padding:20px;margin-bottom:20px}table{width:100%;border-collapse:collapse}th,td{padding:10px;border-bottom:1px solid #334155;text-align:left;vertical-align:top}th{background:#1e293b}.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin:24px 0}.card{background:#111827;border:1px solid #334155;border-radius:12px;padding:16px}.card-title{display:block;color:#94a3b8;margin-bottom:8px}.good strong{color:#4ade80}.warn strong{color:#fbbf24}.bad strong{color:#f87171}.badge{display:inline-block;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:600}.badge.passed{background:#14532d;color:#bbf7d0}.badge.failed{background:#7f1d1d;color:#fecaca}code{color:#cbd5e1}</style></head><body><main>');
    Lines.Add('<section>');
    Lines.Add('<h1>DX.Comply Human-Readable Compliance Report</h1>');
    Lines.Add(Format('<p><strong>Project:</strong> %s<br><strong>SBOM:</strong> %s<br><strong>Generated:</strong> %s</p>', [
      EscapeHtml(SafeText(AData.ProjectInfo.ProjectName)),
      EscapeHtml(AData.SbomOutputPath),
      EscapeHtml(SafeText(AData.Metadata.Timestamp, AData.CompositionEvidence.GeneratedAt))]));
    Lines.Add('</section>');
